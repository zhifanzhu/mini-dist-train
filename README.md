...
Thus I prompted Codex to come up with this codebase so that I implement the core components myself.
The exercises are meant to be educational, rather than production-ready.
Ideally these exercise can solved within a weekend (but I took longer).

There could be two ways to use this repository: i) directly solve the exercises [hardcore exericsing]. ii) prompt Codex/Claude with this full codebase and ask them to explain the concepts.
I've personally solved ex00-ex09, and I did learn a lot.

- The branch `main-testcases` is the starting point, and it contains fresh problem questions without solutions.
- This current branch `dev/mysolutions` is my solution.
- The branch`main-codex-solutions` contains Codex solutions.

Note that, the whole codebase -- e.g. the codes, the other secions in this readme, the two branches -- is generated by Codex.
The only human written texts and codes are this `Author words` section, and my solutions in the exericses,

Personally, I think the problem design, including necessary hints, are still not as great as other excellent online courses like MIT 6.828 and 6.824. 
Sometimes the provided context is too less, sometimes the learning target itself is tedious.

### My Progress 

Finished ex00-ex09

Skipped ex10 as it is not a good exercise: too much niche pytorch mechanisms.
